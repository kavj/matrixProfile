...
int main(int argc, char* argv[]){
    ...
    double* T = NULL;
    posix_memalign((void*)&T,32,n*sizeof(double));
    for(int i = 0; i < n; i++){
        fscanf(f,"%lf\n",&T[i]);
    }    
    printf("check1\n");
    ...
    double* dX = NULL;
    double* dF = NULL;
    posix_memalign((void*)&mu,32,4*n*sizeof(double));
    posix_memalign((void*)&buffer,32,8*n*sizeof(double));
    posix_memalign((void*)&sigmaInv,32,4*n*sizeof(double));
    posix_memalign((void*)&dX,32,4*n*sizeof(double));
    posix_memalign((void*)&dF,32,4*n*sizeof(double));
...
//    double* sigmaInv = malloc(2*n*sizeof(double));
    ...
    clock_t t4 = clock();
    // accumTest(T, dX, dF, sigmaInv, buffer, n,m);
    clock_t t5 = clock();
    /*for(int j = 0; j < 100; j++){
    for(int i = 0; i < 1000; i++){
       avtest5(T,mu,sigmaInv,buffer,m,n,m); 
    }
    }*/
    clock_t t6 = clock();
    accumTest4(T,dX,dF,sigmaInv,buffer,n,m);
    clock_t t7 = clock();//omp_get_wtime();
    //#pragma omp parallel for
/*    for(int i = 0; i < 100000; i++){
        avtest5(T,mu,sigmaInv,buffer,m,n,m);
        //nonav(T,mu,buffer,m,n,m);
    }*/
   // accumTest2(T,mu,sigmaInv,sigmaInv+m,buffer,n,m);
