...
static inline void fma_4x4(vtf &c1, vtf& c2, vtf &c3, vtf &c4, double* dg, double *dh, int i, int j){
...
// there should be a separate reduce rows and reduce columns
...
static void reduce_kern(double *a, double *output, long *outputi, int diag, int offset,int n, int m){
    ...
    for(int k = 0; k < 2048; k += 16){
        ...
        seti_4x4(i1,i2,i3,i4,ca,cb,cc,cd,diag*4+offset*8*k);
    }
